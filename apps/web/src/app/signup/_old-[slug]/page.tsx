...
export default function VolunteerSignupPage() {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const response = await fetch(`/api/signup/${slug}`);
      if (!response.ok) {
  ...
  // const hasPrev = currentIndex < siblingEvents.length - 1 && currentIndex !== -1;
          ...
          {siblingEvents.length > 1 && (
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
              ...
              <select
                id="event-select"
                value={slug}
                onChange={(e) => (window.location.href = `/signup/${e.target.value}`)}
